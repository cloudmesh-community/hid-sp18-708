...
from sklearn import datasets # added following to overcome deprecation issues:
from sklearn import grid_search     # sklearn.grid_search also deprecated; also replaced with the above
...
from sklearn.model_selection import train_test_split      # "from sklearn.model_selection import train_test_split" is deprecated; use the following instead:
from sklearn.model_selection import* # deleted sklearn "datasetsall" import request; doesn't seem to exist anymore
from sklearn.neural_network import MLPClassifier
from sklearn.preprocessing import Imputer 
from sklearn.preprocessing import LabelEncoder
# "from sklearn.model_selection import train_test_split" is deprecated; use the following instead:
from sklearn.model_selection import train_test_split
# sklearn.grid_search also deprecated; also replaced with the above
...
cols = ['sentence','sentiment','polarity']            
...
trainX,testX,trainY,testY = train_test_split(X,y,train_size=.4, test_size=.6)
...
XGB_model = xgb.XGBClassifier(objective='binary:logistic', n_jobs=2, nthread=4,
                              colsample_bytree=0.5)
param_grid = {'max_depth':[4,5], 'learning_rate': [0.2, 0.3]}
model = grid_search.GridSearchCV(estimator=XGB_model,
                                 param_grid=param_grid, scoring='accuracy', verbose=10, 
                                 iid=False, refit=True, cv=4)
...
df_test = pd.merge(df_test.loc[:,['polarity']], df_train, how='left', 
...
print ("Finding highest probable destination for future bookings")
ids = [] #list of ids
...
df = pd.DataFrame(np.column_stack((ids, cts)), columns=['id', 'sentence', 'polarity'])
